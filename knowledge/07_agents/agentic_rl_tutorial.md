...
> 💡 **10 句话搞定 Agentic RL** — RL for LLM agents 是 2024-2026 把 reasoning RL 推向真实工具使用、Web、代码与 GUI 的核心范式（详见 §1-§9 推导 + §10 25 高频题）。
...
10. **2026 的新增接口视角**：Agentic RL 不再只是在 PPO/GRPO loss 上做文章，而是分成五个可替换接口：rollout（APPO / T2PO）、credit（Orchard / PiCA）、skill（SDAR / SLIM）、reward（SRL / ToolOmni / OpenWebRL）、aux loss（AKBE）。如果 terminal reward 广播导致 credit 错分，优先读 [Agentic RL Credit Assignment](agentic_rl_credit_assignment_tutorial.html)。
...
### 1.4　入门实践路线：从 Search 到 Stateful Environment

如果第一次复现 Agentic RL，不建议直接上 OSWorld / SWE-bench 级别的真实环境。更稳的路线是三步：

| 阶段 | 推荐读法 | 训练对象 | reward / 验收重点 |
|---|---|---|---|
| **RAGEN / StarPO** | 先建立系统视角：multi-turn rollout、group sampling、trajectory-level reward、collapse 防护 | 通用 agent trajectory | reward variance、trajectory length、group diversity |
| **Search-R1** | 第一个最小可跑案例：模型自己决定何时 search、搜什么、何时回答 | search query / answer tokens | `state_masking=true`，检索结果是 observation，不能训练模型生成 `<information>` |
| **Long-Horizon Interactive Agents / AppWorld 类任务** | 再进入 stateful digital environment：数据库、REPL、API、应用状态会被 action 改变 | tool call + environment mutation | 最终环境状态满足任务，且没有多余副作用 |

Search-R1 与传统 RAG 的关键区别不是“有没有检索”，而是 **谁决定检索**。传统 RAG 往往由系统先 retrieve，再把文档交给模型；Search-R1 让 search query 成为 policy action，检索结果成为 environment observation，训练才真正变成 trajectory optimization。

AppWorld 类 long-horizon 任务再往前一步：action 不只是读信息，还会真实改变环境（发邮件、改日历、写数据库）。因此 reward 不能只看 final answer，而要验收 final state 与 side effects。这个阶段最容易暴露 credit assignment 问题：某一步好搜索可能被后续误操作拖累，某个多余 tool call 也可能因为最后成功而被错误强化。
...
### 8.7　2026 Agentic RL 算法接口更新

2026 年的 agentic RL 工作越来越像“在 pipeline 的不同接口做局部手术”，而不是单纯替换 PPO / GRPO。按接口看最清楚：

| 接口 | 代表方法 | 改动位置 | 一句话记忆 |
|---|---|---|---|
| **Rollout / intrinsic signal** | **APPO**, **T2PO** | `generate()` 内部、分叉采样、重采样策略 | 把 rollout budget 花在 token / turn 不确定性最高、未来差异最大的地方 |
| **Credit assignment** | **Orchard**, **PiCA** | terminal reward 到 step reward / advantage 的映射 | 把失败轨迹回溯成监督信号，或用 step-PRM 找 pivot step |
| **Skill signal** | **SDAR**, **SLIM** | skill prompt、skill library、蒸馏辅助损失 | 把外部 skill 说明书压进权重，或把 skill 库本身作为可更新对象 |
| **Reward design** | **SRL**, **ToolOmni**, **OpenWebRL** | reward 形状、组合方式、judge 来源 | 把 0/1 reward 变连续、可分段、多目标或自蒸 judge |
| **Auxiliary loss** | **AKBE** | RL 主损失旁路的 SFT-style loss | 用双路 rollout 找“该不该用工具”的知识边界，少量 aux loss 教节制 |

这些方法的共同背景是：terminal outcome reward 虽然可靠，但对 long-horizon agent 太稀疏。新接口的目标是让训练信号更可分、更稠密，或者让采样本身更有信息量。

**与本章主线的关系**：

- APPO / T2PO 更像 rollout 层的 credit 预处理：先找到最值得分叉或重采的位置。
- Orchard / PiCA 直接处理 credit 错分：把“整条轨迹成败”局部化到关键 step。
- SRL / ToolOmni / OpenWebRL 改 reward：让 group 内不再经常全 0 / 全 1。
- SDAR / SLIM 把 skill 当训练变量：不只是 prompt 拼接，而是参与 epoch 更新。
- AKBE 用 aux loss 补主 reward：尤其适合“模型知道答案时不该乱调工具”的成本控制。

> **读文献顺序建议**：先读 Search-R1 / RAGEN 建 baseline，再读 credit assignment 专章（HGPO / SPA-RL / AgentPRM / ARPO / IGPO），最后按接口补 APPO、T2PO、PiCA、ToolOmni、AKBE 等新工作。`algorithms.html` 中部分深读链接在本地目录未出现，因此本节只采用总览级别结论。
