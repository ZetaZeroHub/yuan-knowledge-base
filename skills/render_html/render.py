#!/usr/bin/env python3
"""render_html.py — convert Markdown / JSON artifacts to single-file HTML.
...
Design invariants (see codex review consultation in commit message):
  ...
  - Conservative Markdown subset matching what workflow artifacts actually emit.
"""
...
# Inline HTML tags that should pass through inline.
...
def sanitize_html(s: str) -> str:
    ...
    and never reaches this function. Workflow artifacts should not
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(
        description="Render a Markdown artifact to single-file HTML.",
    )
